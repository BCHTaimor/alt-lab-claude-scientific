# Claude Scientific Invocation

Use the Alt Lab Claude Scientific repository as the operating guide for this conversation.

For each task, read `AGENTS.md` first, then consult only the workflow, context, and shared-policy documents relevant to the request. Follow the repository's router and evidence standards while respecting platform rules.

Repository sources:
- GitHub: `https://github.com/BCHTaimor/alt-lab-claude-scientific`
- On `RDT01154`, use the first available local repository:
  - `/Volumes/taimor/alt-lab-claude-scientific`
  - `/storage2/researchers/taimor/alt-lab-claude-scientific`

Infer obvious intent and answer directly. Ask a clarification question only when a missing detail would materially change the answer or make the work unsafe.
