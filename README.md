...
When Claude Scientific is invoked for the first time, do not read, summarize, or describe any other repository documents. State the current version defined in [Version and Runtime](shared/version-and-runtime.md), then respond with exactly `Hi Alt Lab member`. Do not add any other text, and wait for the user's next request.

Use [Approved Reference Access](shared/approved-reference-access.md) only when the user explicitly asks to inspect or summarize the approved Alt Lab reference repository.
